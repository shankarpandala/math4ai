...
/**
 * Very lightweight syntax highlighter for Python.
 ...
 *   runnable  {bool}     Show "Open in Colab" button
 */
function PythonCode({ code = '', language = 'python', title, runnable = false }) {
  ...
  // Build Colab URL with the code as a gist URL placeholder
  const colabUrl = `https://colab.research.google.com/#create=true`;

  // Tokenize for Python; for other languages just render plain text
  const renderContent = () => {
    if (language === 'python') {
        ...
        <div className="flex items-center gap-2">
          ...
          {runnable && (
            <a
              ...
              className="flex items-center gap-1.5 rounded-md border border-amber-600/50 bg-amber-900/30 px-2.5 py-1 text-xs font-medium text-amber-400 transition-colors hover:bg-amber-900/50"
            >
